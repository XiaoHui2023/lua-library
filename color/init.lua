...
local function assert_color(co, name)
    assert(M.is_color(co), name .. " must be a color")
end

---@param value any
---@return boolean
M.is_color = function(value)
...
---@return number
M.distance = function(color_1, color_2)
...
---@param blue integer
---@return lib.color
M.define = function(red, green, blue)
    assert_byte(red, "red")
...
M.from_hex = function(hex)
    ...
    return M.define(tonumber(red, 16), tonumber(green, 16), tonumber(blue, 16))
end

---@param co lib.color
---@return string
M.to_hex = function(co)
    assert_color(co, "co")
    return string.format("#%02X%02X%02X", co.red, co.green, co.blue)
end

M.RED = M.define(255, 3, 3)
M.BLUE = M.define(0, 126, 255)
M.CYAN = M.define(28, 230, 185)
M.PURPLE = M.define(84, 0, 129)
M.YELLOW = M.define(255, 252, 1)
M.ORANGE = M.define(254, 138, 14)
M.GREEN = M.define(32, 192, 0)
M.PINK = M.define(229, 91, 176)
M.GRAY = M.define(149, 150, 151)
M.LIGHT_GRAY = M.define(200, 200, 200)
M.LIGHT_BLUE = M.define(126, 191, 241)
M.LIGHT_GREEN = M.define(102, 255, 153)
M.DARK_GREEN = M.define(16, 98, 70)
M.BROWN = M.define(78, 42, 4)
M.WHITE = M.define(255, 255, 255)
M.BLACK = M.define(0, 0, 0)
...
M.nearest = function(co, map)
    ...
    local best_distance = M.distance(co, best)

    for i = 2, #map do
        local candidate = map[i]
        assert_color(candidate, "map[" .. i .. "]")
        local candidate_distance = M.distance(co, candidate)
...
M.add_pattern = function(pattern)
    ...
    table.insert(M.PATTERNS, pattern)
end

M.clear_patterns = function()
    for i = #M.PATTERNS, 1, -1 do
        M.PATTERNS[i] = nil
    end
end
