...
- 每个顶层库目录都必须有自己的 `README.md`；新增库目录或重命名库目录时，同轮补齐或迁移该目录 README，并让根 `README.md` 的模块索引保持一致。

## 备忘与待定

- 子库目录命名、包管理与测试方式待后续决议。
- 当前顶层库模块：`callback/`、`color/`、`damage/`、`debugx/`、`list/`、`mathx/`、`metatablex/`、`module/`、`motion/`、`point/`、`reactive/`、`requirex/`、`state_machine/`、`stringx/`、`tablex/`、`template/`。
